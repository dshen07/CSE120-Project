...
public class KThread {
	...
	public static void finish() {
		...
		if (KThread.joinThread.containsKey(currentThread)) {
			KThread.joinThread.get(currentThread).ready();
		}
	...
	public void join() {
		...
		Lib.assertTrue(!(KThread.joinThread.containsKey(this)));
		Lib.assertTrue(this != currentThread);
		...
		// 	this.ready();
		// }
		KThread.joinThread.put(this, KThread.currentThread());
	...
	private static void threadTerminatesNoJoin() {
		KThread child1 = new KThread(new Runnable() {
			public void run() {
				System.out.println("I (heart) Nachos!");
			}
		...
		// We want the child to finish wihout calling join. Although
		// our solutions to the problems cannot busy wait, our test
		// programs can!
		...
		System.out.println("child1 should still terminate without calling join.");
		System.out.println("is it? " + (child1.status == statusFinished));
		Lib.assertTrue((child1.status == statusFinished), " Expected child1 to be finished.");
	}

	private static void currentThreadJoinsItself() {
		// KThread child1 = new KThread(new Runnable() {
		// public void run() {
		// System.out.println("I (heart) Nachos!");
		// }
	...
	// how to get a thread to call join on itself?
	// private static void threadJoinsItself() {
	// KThread child1 = new KThread(new Runnable() {
	// public void run() {
	// System.out.println("I (heart) Nachos!");
	// }
	...
	// System.out.println("After joining, child1 should be finished.");
	// System.out.println("is it? " + (child1.status == statusFinished));
	// Lib.assertTrue((child1.status == statusFinished), " Expected child1 to be
	// finished.");
	// }
	...
	public static void joinTest2() {
		...
		child1.setName("child1").fork();
		new KThread(new Testing(child1)).setName("forked thread").fork();
		child1.join();
		// We want the child to finish before we call join. Although
		// our solutions to the problems cannot busy wait, our test
		// programs can!
	...
	private static void joinCalledMoreThanOnce() {
		KThread child1 = new KThread(new Runnable() {
			public void run() {
				System.out.println("I (heart) Nachos!");
			}
		...
		KThread child2 = new KThread(new Runnable() {
			public void run() {
				System.out.println("Calling Join on child1");
				child1.join();
			}
		});
	...
	private static void joinInSuccession() {
		KThread child1 = new KThread(new Runnable() {
			public void run() {
				System.out.println("I (heart) Nachos!");
			}
		...
		KThread child2 = new KThread(new Runnable() {
			public void run() {
				System.out.println("I (heart) Avacados!");
				child1.fork();
			}
		});
		...
		KThread child3 = new KThread(new Runnable() {
			public void run() {
				System.out.println("I (heart) Apples!");
				child2.join();
			}
		});
	...
	private static void indepPairsOfThreadsJoin() {
		KThread child1 = new KThread(new Runnable() {
			public void run() {
				System.out.println("I (heart) Nachos!");
			}
		...
		// public void run() {
		// child3.join();
		// System.out.println("I (heart) Mangos!");
		// }
		// });
	...
	public static void selfTest() {
		...
		// currentThreadJoinsItself();
		// threadJoinsItself();
		 threadTerminatesNoJoin();
		// joinCalledMoreThanOnce();
		 //joinInSuccession();
		//indepPairsOfThreadsJoin();
	}
	...
	private static class Testing implements Runnable {
		Testing(KThread thread) {
			this.thread = thread;
		}

		public void run() {
			this.thread = new KThread(new PingTest(1)).setName("forked thread");
			thread.fork();
			thread.join();
	...
	public static HashMap<KThread, KThread> joinThread = new HashMap<>();
}
